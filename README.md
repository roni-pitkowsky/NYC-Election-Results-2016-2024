<!-- Inspired by: https://github.com/HartoniAgungPermana/tableau-hr-analytics-dashboard?tab=readme-ov-file -->

# NYC-Election-Results-2016-2024
### [🌐 View the Interactive Dashboard on Tableau Public](https://public.tableau.com/app/profile/roni.pitkowsky/viz/NYCElectionResults2016-2024/TotalVotesDashboard)
<!-- Include small gif here -->
<img width="1799" height="899" alt="2024 Total Votes Dashboard" src="https://github.com/user-attachments/assets/4c937ea8-9b27-4450-8438-8a6ec8f6f448" /><br/>

# 📌 Project Overview
This project compares and analyzes NYC Presidential election results over the course of three elections: 2016, 2020 and 2024.

# 📂 Dataset
This project uses election results data from the NYC Board of Elections and geographic data from the NYC Department of City Planning.<br/>
🗳️ [2024 Results](https://vote.nyc/page/election-results-summary-2024)<br/>
🗳️ [2020 Results](https://vote.nyc/page/election-results-summary-2020)<br/>
🗳️ [2016 Results](https://vote.nyc/page/election-results-summary-2016)<br/>
🗺️ [Election Districts](https://www.nyc.gov/content/planning/pages/resources/datasets/election-districts)
<!-- # 💼 Executive Summary -->

# 🖥️ How to Use
1. Access the Dashboard
    - 🌐 View on Tableau Public → [NYC Election Results 2016-2024](https://public.tableau.com/app/profile/roni.pitkowsky/viz/NYCElectionResults2016-2024/TotalVotesDashboard)
    - 💾 Download the .twbx file → [NYC Election Results 2016-2024.twbx](https://github.com/roni-pitkowsky/NYC-Election-Results-2016-2024/blob/main/NYC%20Election%20Results%202016-2024.twbx)
2. Interact with the Dashboard
    - 📅 Select an election year using the Year Selection menu.
    - 🖱️ Hover over an election district to view detailed district results.
    - ⏩ Switch between different election years using the Year Selection menu.
